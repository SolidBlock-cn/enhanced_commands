...
- [简单方块谓词：`方块id`](simple/zh.md)：简单地匹配方块的 ID 和标签。
- [标签方块谓词：`#标签id`](tag/zh.md)：检测方块是否拥有特定的标签。
- [属性方块谓词：`[属性=值]`](properties/zh.md)：检测方块是否拥有特定的属性。
- [NBT 方块谓词：`{}`](nbt/zh.md)：检测方块的 NBT 数据。
- [属性 NBT 组合：`方块id[...]{...}`](property_nbt_combination/zh.md)：同时检测方块的 ID 或标签、方块状态属性以及 NBT，这相当于原版的用法。
- 恒定方块谓词：`*`：匹配所有方块，始终通过。
- [竖直偏移方块谓词：`>` 和 `<`](horizontal_offset/zh.md)：检测方块的竖直偏移的位置的方块是否符合谓词。
- [引用方块谓词：`$`](reference/zh.md)：引用数据包中定义的方块谓词。
