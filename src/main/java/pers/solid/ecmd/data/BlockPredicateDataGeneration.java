...
public interface BlockPredicateDataGeneration extends DynamicRegistryGenerationBridge<BlockPredicate> {
  ...
  default void configureBridge(ContextBridge<BlockPredicate> context) {
    ...
    context.add(of("typical_noise"), new NoiseBlockPredicate(new WeightedList.Uniform<>(ConstantBlockPredicate.ALWAYS_TRUE, ConstantBlockPredicate.ALWAYS_FALSE), OptionalLong.empty(), new NormalNoise.NoiseParameters(-1, Noise.DEFAULT_AMPLITUDES), Noise.UNIT, Vec3.ZERO));
