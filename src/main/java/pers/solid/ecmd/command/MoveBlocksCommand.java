...
public enum MoveBlocksCommand implements CommandRegistrationCallbackBridge {
  INSTANCE;
    ...
    final BlockTransformationCommand blockTransformationCommand = new BlockTransformationCommand() {
      ...
      public void notifyCompletion(CommandSourceStack source, int affectedBlocks, int affectedEntities) {
        if (affectedEntities == -1) {
          source.sendFeedback$ecBridge(() -> relativePos.map(pair -> Component.translatable("enhanced_commands.commands.moveblocks.complete.direction", Integer.toString(pair.rightInt()), TextUtil.wrapDirection(pair.left()), Integer.toString(affectedBlocks)).enhanced$$(), vec3i -> Component.translatable("enhanced_commands.commands.moveblocks.complete.vector", TextUtil.wrapVector(vec3i), Integer.toString(affectedBlocks)).enhanced$$()), true);
        } else {
          source.sendFeedback$ecBridge(() -> relativePos.map(pair -> Component.translatable("enhanced_commands.commands.moveblocks.complete_with_entities.direction", Integer.toString(pair.rightInt()), TextUtil.wrapDirection(pair.left()), Integer.toString(affectedBlocks), Integer.toString(affectedEntities)).enhanced$$(), vec3i -> Component.translatable("enhanced_commands.commands.moveblocks.complete_with_entities.vector", TextUtil.wrapVector(vec3i), Integer.toString(affectedBlocks), Integer.toString(affectedEntities)).enhanced$$()), true);
        }
      }

      @Override
      public MutableComponent getIteratorTaskName(Region region) {
        return relativePos.map(pair -> Component.translatable("enhanced_commands.commands.moveblocks.task.direction", region.expressAsString(), Integer.toString(pair.rightInt()), TextUtil.wrapDirection(pair.left())), vec3i -> Component.translatable("enhanced_commands.commands.moveblocks.task.vector", region.expressAsString(), TextUtil.wrapVector(vec3i)));
      }
    };
