...
public enum RotateBlocksCommand implements CommandRegistrationCallbackBridge {
  INSTANCE;
  ...
  public static int executeRotate(@Nullable AxisAngle4d axisAngle4d, KeywordArgs keywordArgs, CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
    ...
    final BlockPos pivot = keywordArgs.<Coordinates>getRequiredArg("pivot").getBlockPos(context.getSource());
    ...
    final BlockTransformationCommand blockTransformationCommand = new BlockTransformationCommand() {
      ...
      public void notifyCompletion(CommandSourceStack source, int affectedBlocks, int affectedEntities) {
        if (affectedEntities == -1) {
          source.sendFeedback$ecBridge(() -> Component.translatable("enhanced_commands.commands.rotateblocks.complete", Integer.toString(affectedBlocks)).enhanced$$(), true);
        } else {
          source.sendFeedback$ecBridge(() -> Component.translatable("enhanced_commands.commands.rotateblocks.complete_with_entities", Integer.toString(affectedBlocks), Integer.toString(affectedEntities)).enhanced$$(), true);
        }
      }

      @Override
      public MutableComponent getIteratorTaskName(Region region) {
        return Component.translatable("enhanced_commands.commands.rotateblocks.task", region.expressAsString());
      }
    };
