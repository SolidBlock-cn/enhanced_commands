...
public record TagBlockFunction(HolderSet<Block> tag, List<PropertyNameFunction> properties) implements BlockFunction {
  public static final Codec<TagBlockFunction> STRING_BASED_CODEC = TagKey.hashedCodec(Registries.BLOCK).flatXmap(blockTagKey -> BuiltInRegistries.BLOCK.get(blockTagKey).map(holders -> DataResult.success((HolderSet<Block>) holders)).orElseGet(() -> DataResult.error(() -> "unknown tag: " + blockTagKey.location())), holders -> holders.unwrapKey().map(DataResult::success).orElseGet(() -> DataResult.error(() -> "unknown tag"))).flatComapMap(TagBlockFunction::new, tagBlockFunction -> tagBlockFunction.properties.isEmpty() ? DataResult.success(tagBlockFunction.tag) : DataResult.error(() -> "cannot serialize predicate with properties to strings"));
