...
public record PackratTermFromParser<T>(Atom<T> atom, Parser<T> parser) implements Term<StringReader> {
  @Override
  public boolean parse(ParseState<StringReader> parseState, Scope scope, Control control) {
    ...
    final ParseContext<Object> parseContext = new ParseContext<>(input);
    final @Nullable T parse;
    try {
      parse = parser.parse(parseContext);
    } catch (CommandSyntaxException e) {
      parseState.errorCollector().store(e.getCursor(), new EnhancedSuggestionSupplier<>(parseContext::buildSuggestions), e);
      return false;
    }
