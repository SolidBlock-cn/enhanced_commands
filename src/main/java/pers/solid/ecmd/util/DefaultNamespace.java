...
public class DefaultNamespace {
  ...
  private <T> Codec<Holder.Reference<T>> referenceHolderWithLifecycleForRegistry(Registry<T> registry, boolean simpleByDefault) {
    Codec<Holder.Reference<T>> codec = idCodec(simpleByDefault).comapFlatMap((resourceLocation) -> registry.getHolder(resourceLocation).map(DataResult::success).orElseGet(() -> DataResult.error(() -> "Unknown registry key in " + registry.key() + ": " + resourceLocation)), (reference) -> reference.key().location());
