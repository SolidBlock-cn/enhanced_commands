...
public final class VertexUtil {
  ...
  public static void drawUnitBox(MatrixStack matrices, VertexConsumer vertexConsumer, int x, int y, int z, float red, float green, float blue, float alpha) {
    WorldRenderer.drawBox(matrices, vertexConsumer, x, y, z, x + 1.0d, y + 1.0d, z + 1.0d, red, green, blue, alpha);
  }
   ...
   * 绘制一个以指定坐标为中心的单位立方体。
   *
   * @implNote {@code VertexRendering} 是于 1.21.2 加入的，在此之前是 {@link WorldRenderer}。
   */
  public static void drawUnitBoxCentered(MatrixStack matrices, VertexConsumer vertexConsumer, double x, double y, double z, float red, float green, float blue, float alpha) {
    WorldRenderer.drawBox(matrices, vertexConsumer, x - 0.5, y - 0.5, z - 0.5, x + 0.5, y + 0.5, z + 0.5, red, green, blue, alpha);
  }

  /**
   * <p>绘制一条连接两点 (x1, y1, z1) 和 (x2, y2, z2) 的直线段，并指定两端的颜色。
   * <p>相比 {@link EntityRenderDispatcher#drawVector}，此方法更灵活，且更加有助于优化内存，因为避免反复创建对象。
   *
   * @implNote {@code VertexRendering} 是于 1.21.2 加入的，在此之前是 {@code EntityRenderDispatcher}。
   * @see EntityRenderDispatcher#drawVector
   */
