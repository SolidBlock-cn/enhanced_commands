...
public final class RegionRendering {
  ...
  public static void renderBlockCuboid(@Nullable Vec3i first, @Nullable Vec3i second, PoseStack matrices, VertexConsumer vertexConsumer, Vec3 cameraPos) {
    ...
    if (first != null && second != null) {
      ...
      LevelRenderer.renderLineBox(matrices, vertexConsumer, x1 - cameraPos.x, y1 - cameraPos.y, z1 - cameraPos.z, x2 - cameraPos.x, y2 - cameraPos.y, z2 - cameraPos.z, 0.9F, 0.9F, 0.9F, 1.0F, 0.5F, 0.5F, 0.5F);
    }
    if (first != null) {
      matrices.pushPose();
