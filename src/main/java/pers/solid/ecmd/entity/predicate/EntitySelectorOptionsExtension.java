  ...
  public static boolean mixinReadMultipleTypes(EntitySelectorParser reader, boolean inverted, @NotNull Either<EntityType<?>, TagKey<EntityType<?>>> already) throws CommandSyntaxException {
    ...
    if (stringReader.canRead() && stringReader.peek() == '|') {
      ...
      while (stringReader.canRead() && stringReader.peek() == '|') {
        ...
        reader.setSuggestions((suggestionsBuilder, suggestionsBuilderConsumer) -> {
          SharedSuggestionProvider.suggestResource(BuiltInRegistries.ENTITY_TYPE.listElements().filter(r -> !parsedTypes.contains(r.value())), suggestionsBuilder, r -> r.key().location(), r -> r.value().getDescription());
          return SharedSuggestionProvider.suggestResource(BuiltInRegistries.ENTITY_TYPE.getTags().map(HolderSet.Named::key).filter(tagKey -> !parsedTypeKeys.contains(tagKey)).map(TagKey::location), suggestionsBuilder, "#");
        });
