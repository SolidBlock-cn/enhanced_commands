...
public enum MirrorArgument implements StringIdentifiable, Function<@NotNull ServerCommandSource, @NotNull BlockMirror> {
  ...
  FORWARD("entityPredicate", source -> Direction.fromHorizontalDegrees(source.getRotation().y).getAxis() == Direction.Axis.X ? BlockMirror.FRONT_BACK : BlockMirror.LEFT_RIGHT),
  SIDE("side", source -> Direction.fromHorizontalDegrees(source.getRotation().y).getAxis() == Direction.Axis.Z ? BlockMirror.FRONT_BACK : BlockMirror.LEFT_RIGHT),
