...
public class SimpleBlockFunctionParser<S> extends SimpleBlockParser<S> {
  ...
  protected <T extends Comparable<T>> void parsePropertyNameValue(Property<T> property, Comparator comparator) throws CommandSyntaxException {
    ...
    if (parse.isPresent()) {
      ...
      throw withCursorEnd(BlockArgumentParser.INVALID_PROPERTY_EXCEPTION.createWithContext(parser.reader, blockId.toString(), property.getName(), valueName), cursorAfterParseValue);
    }
  }
  ...
  protected void addComparatorTypeSuggestions() {
    parseContext.parser().addSuggestion((context, suggestionsBuilder) -> CommandSource.suggestMatching(List.of("=", "=="), suggestionsBuilder));
  }
