...
public record EntityPredicateArgument(CommandBuildContext commandBuildContext) implements ArgumentType<EntityPredicate> {
  private static final List<String> EXAMPLES = List.of("@a", "SolidBlock", "0123", "@r", "@e[distance=..5]", "[m=c]", "[gamemode=creative]");

  public static EntityPredicateArgument entityPredicate(CommandBuildContext commandBuildContext) {
    return new EntityPredicateArgument(commandBuildContext);
  }
  ...
  public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
    if (context.getSource() instanceof SharedSuggestionProvider commandSource) {
      ...
      EntitySelectorParser entitySelectorParser = new EntitySelectorParser(stringReader, commandSource.hasPermission(2));
