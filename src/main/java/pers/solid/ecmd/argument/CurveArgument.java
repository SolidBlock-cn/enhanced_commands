...
public record CurveArgument(CommandBuildContext commandBuildContext) implements ArgumentType<CurveProvider<?>> {
  private static final List<String> EXAMPLES = List.of("straight(~~~, ~3~3~3)", "straight(from ^^^ to ^^^5)", "circle(5)");

  public static CurveArgument curve(CommandBuildContext commandBuildContext) {
    return new CurveArgument(commandBuildContext);
  }

  public static Curve getCurve(CommandContext<CommandSourceStack> context, String name) throws CommandSyntaxException {
    try {
      return context.getArgument(name, CurveProvider.class).toAbsoluteRegion(context.getSource());
  ...
  @Override
  public CurveProvider<?> parse(StringReader reader) throws CommandSyntaxException {
    return CurveProvider.parse(new ParseContext<>(commandBuildContext, reader, false, false));
  }
  ...
  public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
    ...
    try {
      CurveProvider.parse(parseContext);
