...
public record CurveArgumentType(CommandBuildContext commandBuildContext) implements ArgumentType<CurveArgument<?>> {
  private static final List<String> EXAMPLES = List.of("straight(~~~, ~3~3~3)", "straight(from ^^^ to ^^^5)", "circle(5)");

  public static CurveArgumentType curve(CommandBuildContext commandBuildContext) {
    return new CurveArgumentType(commandBuildContext);
  }
  ...
  public CurveArgument<?> parse(StringReader reader) throws CommandSyntaxException {
    return CurveArgument.parse(new ParseContext<>(commandBuildContext, reader, false, false));
  }
