...
public record ValueNamePropertyPredicate(String propertyName, Comparator comparator, String valueName) implements PropertyNamePredicate {
  ...
  public TestResult testAndDescribe(BlockState blockState, BlockPos blockPos) {
    ...
    final Property<?> property = stateManager.getProperty(propertyName);
    if (property == null) {
      return new TestResult(false, Text.translatable(propertyName.isEmpty() ? "enhancedCommands.argument.blockPredicate.no_property_this_name_empty" : "enhancedCommands.argument.blockPredicate.no_property_this_name", blockState.getBlock().getName().styled(EnhancedCommands.STYLE_FOR_TARGET), Text.literal(propertyName).styled(EnhancedCommands.STYLE_FOR_EXPECTED)).formatted(Formatting.RED));
    }
    if (property.parse(valueName).isEmpty()) {
      return new TestResult(false, Text.translatable(valueName.isEmpty() ? "enhancedCommands.argument.blockPredicate.property_value_not_parsed_empty" : "enhancedCommands.argument.blockPredicate.property_value_not_parsed", Text.literal(propertyName).styled(EnhancedCommands.STYLE_FOR_TARGET), Text.literal(valueName).styled(EnhancedCommands.STYLE_FOR_ACTUAL)).formatted(Formatting.RED));
    }
    final boolean test = comparator.parseAndTest(blockState, property, valueName);
    if (test) {
      return new TestResult(true, Text.translatable("enhancedCommands.argument.blockPredicate.property_test_pass", Text.literal(asString()).styled(EnhancedCommands.STYLE_FOR_EXPECTED)).formatted(Formatting.GREEN));
    } else {
      return new TestResult(false, Text.translatable("enhancedCommands.argument.blockPredicate.property_not_this_value", EnhancedCommands.wrapBlockPos(blockPos), propertyAndValue(blockState, property).styled(EnhancedCommands.STYLE_FOR_ACTUAL), Text.literal(asString()).styled(EnhancedCommands.STYLE_FOR_EXPECTED)).formatted(Formatting.RED));
    }
  }
