...
public record RotationPredicateEntry(BridgeFloatRange floatRange, @MagicConstant(stringValues = {"pitch", "yaw"}) String type, ToFloatFunction<Entity> angleFunction, Predicate<Entity> backingPredicate) implements EntityPredicateEntry {
  ...
  public TestResult testAndDescribe(Entity entity, Text displayName) {
    ...
    return TestResult.of(result, Text.translatable("enhanced_commands.entity_predicate." + type + (result ? ".in_range" : ".out_of_range"), displayName, TextUtil.literal(angle).styled(Styles.ACTUAL), Text.literal(floatRange.asString()).styled(Styles.EXPECTED)));
  }
