...
public record IdReplaceBlockFunction(Pattern pattern, String replacement) implements BlockFunction {
  ...
  public @NotNull BlockState getModifiedState(BlockState blockState, BlockState origState, Level world, BlockPos pos, MutableObject<CompoundTag> blockEntityData, BlockFunctionContext context) {
    ...
    return world.registryAccess().lookupOrThrow(Registries.BLOCK).getOptional(identifier).filter(block1 -> block1.isEnabled(world.enabledFeatures())).map(Block::defaultBlockState).orElse(blockState);
  }
