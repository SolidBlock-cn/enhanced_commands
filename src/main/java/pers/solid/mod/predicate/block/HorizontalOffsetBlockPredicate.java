...
public record HorizontalOffsetBlockPredicate(int offset, BlockPredicate blockPredicate) implements BlockPredicate {
  public static final Text ABOVE_BLOCK = Text.translatable("argument.ecBlockStatePredicate.above_block");
  ...
  public TestResult testAndDescribe(CachedBlockPosition cachedBlockPosition) {
    ...
    if (offset > 0) {
      description = Text.translatable("blockPredicate.test_relative_above_" + string, offset, EnhancedCommands.wrapBlockPos(cachedBlockPosition.getBlockPos())).formatted(formatting);
    } else {
      description = Text.translatable("blockPredicate.test_relative_below_" + string, -offset, EnhancedCommands.wrapBlockPos(cachedBlockPosition.getBlockPos())).formatted(formatting);
    }
  ...
  public enum Type implements BlockPredicateType<HorizontalOffsetBlockPredicate> {
    ...
    public @Nullable BlockPredicate parse(ArgumentParser parser) throws CommandSyntaxException {
      ...
      if (offset != 0) {
        return new HorizontalOffsetBlockPredicate(offset, BlockPredicate.parse(parser));
      } else if (prefixed) {
